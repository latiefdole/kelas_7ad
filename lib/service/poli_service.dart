import 'package:dio/dio.dart';
import '../helpers/api_client_1.dart';
import '../model/poli.dart';

class PoliService {
  Future<List<Poli>> listData() async {
    final Response response = await ApiClientPertama().get('poli');
    final List data = response.data as List;
    List<Poli> result = data.map((json) => Poli.fromJson(json)).toList();
    return result;
  }

  Future<Poli> simpan(Poli poli) async {
    var data = poli.toJson();
    final Response response = await ApiClientPertama().post('poli', data);
    Poli result = Poli.fromJson(response.data);
    return result;
  }

  Future<Poli> ubah(Poli poli, String id) async {
    var data = poli.toJson();
    final Response response = await ApiClientPertama().put('poli/$id', data);
    print(response);
    Poli result = Poli.fromJson(response.data);
    return result;
  }

  Future<Poli> getById(String id) async {
    final Response response = await ApiClientPertama().get('poli/$id');
    Poli result = Poli.fromJson(response.data);
    return result;
  }

  Future<bool> hapus(Poli poli) async {
    final Response response =
        await ApiClientPertama().delete('poli/${poli.id}');
    Poli result = Poli.fromJson(response.data);
    if (response.statusCode == 200) {
      return true;
    } else {
      return false;
    }
  }
}
